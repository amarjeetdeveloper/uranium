...
const updatedBookPrice = async function(req, res){
    const bookDetails = await bookModel.find( { name:"Two ststes" } )
    const id = bookDetails[0].author_id
    const authorNam = await authorModel.find( { author_id:id } ).select( { author_name:1, _id:0})

    const bkName = bookDetails[0].name
    const updatedPrice = await bookModel.findOneAndUpdate({name:bkNmae},{price:100},{new:true}).select({price:1, _id:0})
