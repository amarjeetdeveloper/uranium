...
const createBook= async function (req, res) {
    let data= req.body
                    //   3
    let savedData= await BookModel.find({isPublished: true })
//   let savedData= await BookModel.createBook()
...
const getBooksData= async function (req, res) {
                                                //  2
//  let allBooks= await BookModel.find( { authorName : "Miguel de Cervantes" , bookName: "  Don Quixote " } )

            //    4
//  let allBooks= await BookModel.find( { sales : 50} )
// let allBooks= await BookModel.find( {isPublished: true} )
//    let allBooks= await BookModel.find( {bookName: "  Don Quixote "} )
